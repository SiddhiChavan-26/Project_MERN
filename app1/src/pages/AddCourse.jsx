  ...
  const [expireDays, setExpireDays] = useState("")
    ...
    const course = async () => {
        if (courseName == '')
            ...
            toast.warn('startDate must be entered')
        else if (endDate == '')
            toast.warn('endDate must be entered')
        else if (expireDays == '')
            toast.warn('expireDays must be entered')
        else {
            const result = await newCourse(course_name,description,fees,start_date,end_date,video_expire_days)
            if (result.status == 'success') {
                navigate('/')
                toast.success('New course add successfully')
            } else
